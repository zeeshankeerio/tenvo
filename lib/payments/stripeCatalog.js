...
/** Same 15% annual discount as public pricing page. */
export const YEARLY_BILLING_DISCOUNT = 0.85;

/**
 * @param {'monthly' | 'yearly' | string} interval
 * @returns {'monthly' | 'yearly'}
 */
export function normalizeBillingInterval(interval) {
  return String(interval || 'monthly').toLowerCase() === 'yearly' ? 'yearly' : 'monthly';
}

/**
 * @param {number} baseMajor Monthly list price in major units.
 * @param {'monthly' | 'yearly'} interval
 */
export function resolveBillingChargeMajor(baseMajor, interval) {
  const base = Number(baseMajor);
  if (!Number.isFinite(base) || base <= 0) return 0;
  if (interval === 'yearly') {
    return Math.round(base * YEARLY_BILLING_DISCOUNT * 12);
  }
 ...
 * @property {'month' | 'year'} stripeRecurringInterval
...
export function resolveBillingCatalogItem({
  ...
  if (packageKey) {
    ...
    if (!listMajor || listMajor <= 0) return null;
    const chargeMajor = resolveBillingChargeMajor(listMajor, billingInterval);
    const stripeRecurringInterval = billingInterval === 'yearly' ? 'year' : 'month';
    return {
      kind: 'domain_package',
      planTier: tier,
      domainPackageKey: packageKey,
      currency: stripeCurrency,
      unitAmountMinor: toStripeMinorUnits(chargeMajor, stripeCurrency),
      productName: `Tenvo ${pkg.name}`,
      description: pkg.tagline,
      lookupKey: `tenvo_pkg_${packageKey}_${billingInterval}_${stripeCurrency}`,
      billingInterval,
      ...
      metadata: {
        ...
        domainPackageKey: packageKey,
        planTier: tier,
        catalogInterval: billingInterval,
  ...
  const listPrice = getPlanListPrice(tier, { interval: billingInterval });
  const listMajor = cur === 'usd' ? listPrice.usd : listPrice.pkr;
  if (!listMajor || listMajor <= 0) return null;
  const chargeMajor =
    billingInterval === 'yearly'
      ? resolveBillingChargeMajor(
          cur === 'usd' ? PLAN_TIERS[tier].price_usd : PLAN_TIERS[tier].price_pkr,
          billingInterval
        )
      : listMajor;
  const stripeRecurringInterval = billingInterval === 'yearly' ? 'year' : 'month';
  ...
  return {
    ...
    lookupKey: `tenvo_plan_${tier}_${billingInterval}_${stripeCurrency}`,
    billingInterval,
    stripeRecurringInterval,
    metadata: {
      billingKind: 'plan',
...
export function buildCheckoutLineItemFromCatalog(item) {
  return {
    ...
    price_data: {
      ...
      recurring: { interval: item.stripeRecurringInterval || 'month' },
      product_data: {
