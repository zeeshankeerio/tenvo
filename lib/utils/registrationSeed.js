...
/**
 * Registration seed — category shells for most verticals; rich catalogs for auto-parts and vehicle-dealership.
...
export function buildRegistrationSeedPayload({ businessId, domainKey, countryIso }) {
  ...
  if (shouldSeedRichCatalogOnRegistration(canonicalKey)) {
    const knowledge = getDomainKnowledge(canonicalKey, { countryIso: iso });
