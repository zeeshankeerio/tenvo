...
export function buildFashionHomeSections({
  ...
  return {
    variant,
    unstitched: {
      title: variant === 'textile' ? 'FABRIC & SUIT LOTS' : 'UNSTITCHED',
      tiles: unstitchedTiles,
      show: flags.unstitched && unstitchedTiles.length >= 2,
      viewAllHref:
        variant === 'textile'
          ? `${base}/products?search=${encodeURIComponent('lawn')}`
          : `${base}/products?search=${encodeURIComponent('unstitched')}`,
    },
    readyToWear: {
      title: 'READY TO WEAR',
    ...
    offers: {
      title: variant === 'textile' ? 'TRADE OFFERS' : 'OFFERS & SALE',
      products: offers,
    ...
    newArrivals: {
      title: variant === 'textile' ? 'NEW LOTS' : 'NEW ARRIVALS',
