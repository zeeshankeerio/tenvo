...
export async function cacheStorefrontRead(fetchFn, keyParts, options) {
  try {
    ...
    if (error instanceof StorefrontCacheBypassError) {
      return /** @type {T} */ (error.value);
    }
    throw error;
  }
}
...
/** Bypass unstable_cache for values that must not be persisted (e.g. null tenant misses). */
export class StorefrontCacheBypassError extends Error {
  /** @param {unknown} value */
  constructor(value) {
    super('Storefront cache bypass');
    this.name = 'StorefrontCacheBypassError';
    this.value = value;
  }
}
