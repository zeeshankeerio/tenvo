...
export const ProductService = {
        ...
        try {
            ...
            const safeSerialize = (val) => {
                ...
                if (typeof val === 'string') {
                    return val.trim() === '[object Object]' ? '{}' : val;
                }
                try {
            ...
            if (insertData.domain_data !== undefined) {
                insertData.domain_data = safeSerialize(insertData.domain_data);
            }
            if (insertData.unit_conversions !== undefined) {
                insertData.unit_conversions = safeSerialize(insertData.unit_conversions);
            }
    ...
    async updateProduct(id, businessId, updates, txClient = null) {
        ...
        try {
            ...
            if (cleanUpdates.domain_data !== undefined) {
                cleanUpdates.domain_data = safeSerialize(cleanUpdates.domain_data);
            }
            if (cleanUpdates.unit_conversions !== undefined) {
                cleanUpdates.unit_conversions = safeSerialize(cleanUpdates.unit_conversions);
            }
    ...
    sanitizeProduct(product) {
        ...
        return {
            ...
            batches: safeParseJson(product.batches, []),
            serial_numbers: safeParseJson(product.serial_numbers, []),
            stock_locations: safeParseJson(product.stock_locations, []),
