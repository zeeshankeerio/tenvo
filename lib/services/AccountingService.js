...
export const AccountingService = {
        ...
        try {
            ...
            switch (type) {
                ...
                case 'expense': {
                    ...
                    const isPaid = data.paymentStatus === 'paid' || data.isPaid;
                    const isCash = (data.paymentMethod || data.paymentMode || 'cash') === 'cash';
    ...
    async reverseJournalEntry(journalId, context, txClient = null) {
        ...
        try {
            ...
            // 6. Link reversal to original journal
            await client.query(`
                UPDATE journal_entries
                SET reversed_by = $1, updated_at = NOW()
    ...
    async closeFiscalPeriod(periodId, context, txClient = null) {
        ...
        try {
            ...
            `, [userId, notes, periodId, businessId]);
