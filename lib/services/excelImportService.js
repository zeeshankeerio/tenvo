...
export { COLUMN_SYNONYMS, detectColumnMapping, applyColumnMapping, nestMappedDomainData };
...
export function validateImportRow(row, existingProducts = {}, category = 'retail-shop', columnMapping = null) {
  ...
  // Domain columns mapped as domain_data.* (textile article/design, pharmacy DL, …)
  for (const [field, value] of Object.entries(mapped)) {
    if (!field.startsWith('domain_data.')) continue;
    if (value === undefined || value === null || value === '') continue;
    cleaned[field] = value;
  }

  const nested = nestMappedDomainData(cleaned);
