...
export async function setCachedStorefrontBusiness(normalizedDomain, business) {
  ...
  const key = domainIndexKey(normalizedDomain);
  const ok = await redisSetEx(key, payload, REDIS_TTL.domainIndex);
  const canonical = business.domain;
  if (canonical && String(canonical).toLowerCase() !== normalizedDomain) {
    await redisSetEx(domainIndexKey(String(canonical)), payload, REDIS_TTL.domainIndex);
  }
  return ok;
}
