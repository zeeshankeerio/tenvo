...
export async function checkPlanFeature(businessId, featureKey, txClient = null) {
    ...
    const platformOverrides = await loadPlatformFeatureOverridesForBusiness(businessId, txClient);

    if (!planHasFeatureWithPackaging(plan_tier, featureKey, settings, platformOverrides)) {
