...
/**
 * Uncached product list fetch (used by cached getProducts wrapper).
 */
...
export async function getProductBySlug(businessId, slug) {
  if (!businessId || !slug) {
    return actionFailure('INVALID_INPUT', 'Business ID and slug are required');
  }
...
export async function getRelatedProducts(businessId, productId, limit = 8) {
  if (!businessId || !productId) {
    return actionFailure('INVALID_INPUT', 'Business ID and product ID are required');
  }
