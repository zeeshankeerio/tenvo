...
// Verbose per-request storefront lookup logs are opt-in (they otherwise flood
// dev logs since layout + page each resolve the domain on every navigation).
...
export async function getBusinessByDomain(domain) {
  ...
  try {
    ...
    if (STOREFRONT_DEBUG) console.log(`[getBusinessByDomain] Looking for domain: ${normalizedDomain} (alternatives: ${altDomain}, ${altDomain2})`);
    ...
    if (STOREFRONT_DEBUG) console.log(`[getBusinessByDomain] Found business: ${business.business_name} (ID: ${business.id})`);
    ...
    if (dbOff || jsonOff) {
      if (STOREFRONT_DEBUG) console.log(`[getBusinessByDomain] Storefront disabled for business: ${business.id}`);
      return actionFailure('STOREFRONT_DISABLED', 'Storefront is not enabled for this business');
    }

    if (STOREFRONT_DEBUG) console.log(`[getBusinessByDomain] Storefront enabled for business: ${business.id}`);
