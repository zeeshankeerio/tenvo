...
export async function upsertIntegratedProductAction(params) {
    ...
    try {
        ...
        const hasBatchesOrSerials = (batches && batches.length > 0) || (serialNumbers && serialNumbers.length > 0);
        ...
        // 4. Handle Simple Stock (no batches/serials)
            ...
            if (!isUpdate && initialStock > 0) {
                ...
                if (delta > 0) {
                    await addStockAction({
                        businessId, productId: finalProductId, warehouseId: null,
                        quantity: delta, costPrice: productData.cost_price || 0,
                        notes: 'Manual Stock Adjustment', referenceType: 'adjustment', referenceId: finalProductId
                    }, client);
                } else if (delta < 0) {
                    await removeStockAction({
                        businessId, productId: finalProductId, warehouseId: null,
