...
export async function createBusiness(data) {
    ...
    const domainPkgRegistration = buildRegistrationFromDomainPackage(
        ...
        { planTier: normalizedPlanTier, verticalKey: normalizedCategory || undefined }
    );
    ...
    if (!registrationCategory || !domainKnowledge[registrationCategory]) {
    ...
    try {
        ...
        if (isMembershipRelevant(normalizedCategory)) {
            try {
                await MembershipService.syncPlansFromInventory(result.id, normalizedCategory);
        ...
        if (!ownerIsplatform) {
            try {
                const platformOwnerEmails = (process.env.PLATFORM_OWNER_EMAILS || process.env.PLATFORM_OWNER_EMAIL || '')
                    .split(',')
                    .map(e => e.trim())
                    .filter(Boolean);
