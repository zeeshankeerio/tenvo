 ...
 *   1. platformOverrides  — platform admin always wins (absolute control)
 *   2. owner custom packaging (settings.packaging.mode === 'custom') — tenant-level toggle
 *   3. plan tier defaults
 *
 * Note: platform overrides intentionally outrank owner packaging so that platform admins
 * can force-enable or force-disable features regardless of what the tenant has configured.
 *
...
export function planHasFeatureWithPackaging(planTier, featureKey, settings, platformOverrides) {
  ...
  // 1. Platform admin overrides always win
