    ...
    try {
        ...
        if (platformAdmin) {
            ...
            if (planRes.rows.length > 0) {
                ...
                platformFeatureOverrides = await loadPlatformFeatureOverridesForBusiness(businessId, client, session.user.id);
            }
        }
    } finally {
 ...
 * @param {unknown} [options.businessSettings] - Optional `businesses.settings` for packaging
 * @param {Record<string, boolean>} [options.platformOverrides] - Optional platform admin feature overrides
