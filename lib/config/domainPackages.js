...
export function resolveDomainPackageForVertical(verticalKey) {
  ...
  const canonical = String(verticalKey).trim();
  return listDomainPackages().find((pkg) => pkg.verticals.includes(canonical)) || null;
}
...
export function buildRegistrationFromDomainPackage(packageKey, options = {}) {
  ...
  const verticalKey =
    options.verticalKey && pkg.verticals.includes(options.verticalKey)
      ? options.verticalKey
      : pkg.defaultVertical;
