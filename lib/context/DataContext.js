...
export function DataProvider({ children }) {
    ...
    // Prevent duplicate shell bootstrap for the same business (localStorage cache + server sync)
    const shellReadyBusinessIdRef = useRef(null);
    /** Tracks last finance snapshot key so date-range changes refetch without duplicating shell bootstrap. */
    const financeDateKeyRef = useRef(null);

    const SHELL_FETCH_TIMEOUT_MS = 12000;
    ...
    useEffect(() => {
        ...
        Promise.race([shellWork, timeout]).finally(markShellReady);
