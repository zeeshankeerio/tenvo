...
/** Unblock hub shell if analytics bootstrap exceeds this (ms). */
...
export function DataProvider({ children }) {
    ...
    useEffect(() => {
        ...
        // Shell-first: only analytics (single lightweight server action).
        // Finance is heavier (3 parallel actions, 5+ SQL queries) so it runs in background.
        const shellWork = Promise.allSettled([fetchAnalytics()]);
        ...
        Promise.race([shellWork, timeout]).finally(() => {
            ...
            // Background: finance + sales + inventory + purchases + expenses
            Promise.allSettled([
                fetchFinance(),
                fetchSales(),
                fetchInventory(),
