-- Tenant customer membership management (plans, enrollments, benefits).
...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'membership_plans_business_id_fkey') THEN
        ALTER TABLE membership_plans
            ADD CONSTRAINT membership_plans_business_id_fkey
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'membership_plans_product_id_fkey') THEN
        ALTER TABLE membership_plans
            ADD CONSTRAINT membership_plans_product_id_fkey
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'customer_memberships_plan_id_fkey') THEN
        ALTER TABLE customer_memberships
            ADD CONSTRAINT customer_memberships_plan_id_fkey
            FOREIGN KEY (plan_id) REFERENCES membership_plans(id) ON DELETE RESTRICT ON UPDATE NO ACTION;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'customer_memberships_initial_invoice_id_fkey') THEN
        ALTER TABLE customer_memberships
            ADD CONSTRAINT customer_memberships_initial_invoice_id_fkey
            FOREIGN KEY (initial_invoice_id) REFERENCES invoices(id) ON DELETE SET NULL ON UPDATE NO ACTION;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'customer_memberships_recurring_invoice_id_fkey') THEN
        ALTER TABLE customer_memberships
            ADD CONSTRAINT customer_memberships_recurring_invoice_id_fkey
            FOREIGN KEY (recurring_invoice_id) REFERENCES invoices(id) ON DELETE SET NULL ON UPDATE NO ACTION;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'membership_events_business_id_fkey') THEN
        ALTER TABLE membership_events
            ADD CONSTRAINT membership_events_business_id_fkey
            FOREIGN KEY (business_id) REFERENCES businesses(id) ON DELETE CASCADE ON UPDATE NO ACTION;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'membership_events_membership_id_fkey') THEN
        ALTER TABLE membership_events
            ADD CONSTRAINT membership_events_membership_id_fkey
            FOREIGN KEY (membership_id) REFERENCES customer_memberships(id) ON DELETE CASCADE ON UPDATE NO ACTION;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'membership_benefits_business_id_fkey') THEN
        ALTER TABLE membership_benefits
            ADD CONSTRAINT membership_benefits_business_id_fkey
            FOREIGN KEY (business_id) REFERENCES businesses(id) ON DELETE CASCADE ON UPDATE NO ACTION;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'membership_benefits_plan_id_fkey') THEN
        ALTER TABLE membership_benefits
            ADD CONSTRAINT membership_benefits_plan_id_fkey
            FOREIGN KEY (plan_id) REFERENCES membership_plans(id) ON DELETE CASCADE ON UPDATE NO ACTION;
    END IF;
END $$;

CREATE UNIQUE INDEX IF NOT EXISTS membership_plans_business_product_key
    ON membership_plans (business_id, product_id)
    WHERE product_id IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_membership_plans_business_vertical
    ON membership_plans (business_id, vertical_key, is_active);

CREATE INDEX IF NOT EXISTS idx_membership_plans_business_slug
    ON membership_plans (business_id, slug);

CREATE INDEX IF NOT EXISTS idx_customer_memberships_business_customer_status
    ON customer_memberships (business_id, customer_id, status);

CREATE INDEX IF NOT EXISTS idx_customer_memberships_business_ends
    ON customer_memberships (business_id, ends_at);

CREATE INDEX IF NOT EXISTS idx_customer_memberships_business_next_billing
    ON customer_memberships (business_id, next_billing_at);

CREATE INDEX IF NOT EXISTS idx_customer_memberships_business_plan
    ON customer_memberships (business_id, plan_id);

CREATE INDEX IF NOT EXISTS idx_membership_events_membership_created
    ON membership_events (membership_id, created_at DESC);

CREATE INDEX IF NOT EXISTS idx_membership_events_business_type
    ON membership_events (business_id, event_type);

CREATE INDEX IF NOT EXISTS idx_membership_benefits_business_plan
    ON membership_benefits (business_id, plan_id);
